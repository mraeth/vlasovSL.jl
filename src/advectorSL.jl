...
function advectX!(f::DistributionGrid1d2v{DT,Polar}, grid::PolarGrid, advector=advect1DFourier!) where DT
    for iv1 = 1:size(f.data)[2]
        for iv2 = 1:size(f.data)[3]
            xdisp = grid.vaxes[1][iv1] * cos(grid.vaxes[2][iv2])
            fshift =  grid.dt * xdisp/ grid.delta[1]
            advector(view(f.data, :,iv1, iv2), fshift, grid)
        end
    end
...
function advectX2!(f::DistributionGrid1d2v, grid::Grid)
    xdisp = map(i->R(-grid.b0*grid.time[grid.index[1]])*[ grid.vaxes[1][i] ,  grid.vaxes[2][i]] , 1:length(grid.vaxes[1]))
...
function advectV2!(f::DistributionGrid1d1v, grid::Grid, shiftArray::Array{Float64,1})
